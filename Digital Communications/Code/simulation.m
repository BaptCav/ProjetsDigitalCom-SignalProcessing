...
for snr_point = 1:length(EbN0_db)
  for forced_errror_index = 1:length(forced_errors)
      ...
      for blk = 1:nr_blocks
        ...
        t_samp = 1+Q*nr_guard_bits/2+Q-1+forced_errors(forced_errror_index); %Force the value of t_samp, we can add error here
        ...
        nr_errors_forced(forced_errror_index) = nr_errors_forced(forced_errror_index) + sum(temp);
